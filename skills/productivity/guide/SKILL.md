...
- **If the marker is present but the user says the source is already rewritten**, route to `to-spec` (spec) or `to-tickets` (ticket — its single-ticket healing mode) to republish it, which clears the marker and releases the gate. In neither case does `spec-execution` run yet — republishing clears the marker; `guide`, the user, and `spec-execution` do not.
