...
9. **Deliver** — commit, push, open a PR whose body says `Closes #N` — the linkage a later run needs to tell whether a blocker actually merged. With no remote there is no push and no PR: it delivers locally, naming the branch and commits, and reports only remote state it has just measured. Never merges — the merge is the human's.
